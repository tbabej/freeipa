...
class sudorule_mod(LDAPUpdate):
    ...
    def pre_callback(self, ldap, dn, entry_attrs, attrs_list, *keys, **options):
        ...
        error = _("%{type} category cannot be set to 'all' "
                  "while there are allowed %{objects}")
        ...
        # Each category needs to check if any of its relevant attributes
        # are set before allowing setting the category to ALL
        def check_all_category(category, member_attrs, error):
        ...
        for category, member_attrs, error in category_info:
            check_all_category(category, member_attrs, error)
